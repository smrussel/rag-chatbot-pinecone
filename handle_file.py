    ...
    try:
        ...
        for loc in root.findall(".//{http://www.sitemaps.org/schemas/sitemap/0.9}loc"):
            ...
            if html:
                ...
                final_text_output = final_text_output + text
                print(f"Text found: {text[:100]}")  # Print first 100 characters of scraped text
            print("-" * 50)
...
def scrape_domain(domain_url):
    print(f"Scraping: {domain_url}")
...
def check_and_process(domain_url):
    try:
        response = requests.get(domain_url + "/sitemap.xml")
        if response.status_code == 200:
            sitemap_url = domain_url + "/sitemap.xml"
            print("Sitemap found.")
            extracted_text = crawl_sitemap(sitemap_url)
            return extracted_text
        else:
            print("Sitemap not found.")
            extracted_text = scrape_domain(domain_url)
            return extracted_text
    except requests.exceptions.RequestException as e:
...
def handle_url(url,session_id, pinecone_index, tokenizer):
    logging.info("[handle_url] Handling url: {}".format(url))
    # file_text_dict = current_app.config["file_text_dict"]
    
    extracted_text =check_and_process(url)
    # file_text_dict[url] = extracted_text
    
    return handle_file_string(url, session_id, extracted_text, pinecone_index, tokenizer)
...
def handle_file(file, session_id, pinecone_index, tokenizer):
    ...
    return handle_file_string(filename, session_id, extracted_text, pinecone_index, tokenizer)
...
def extract_text_from_file(file):
    ...
    if file.mimetype == "application/pdf":
        ...
        # Read JSON file and extract text from its content
        json_data = json.load(file)
        extracted_text = json.dumps(json_data)
    elif file.mimetype == "text/csv":
        # Read text from CSV file using pandas
        df = pd.read_csv(file)
        extracted_text = df.to_string(index=False)
    elif file.mimetype in ["application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]:
        # Read text from Excel file using pandas
...
def handle_file_string(filename, session_id, file_body_string, pinecone_index, tokenizer):
    """Handle a file string by creating embeddings and upserting them to Pinecone."""
    ...
    for i, (text_chunk, embedding) in enumerate(text_embeddings):
        ...
        vectors.append(
            (id, embedding, {"filename": filename,"text":text_chunk, "file_chunk_index": i}))
